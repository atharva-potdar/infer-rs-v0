...
fn test_verify_model_ort() -> anyhow::Result<()> {
    ...
    // Use Tensor::from_array with an explicit shape and boxed slice to avoid requiring ort's ndarray feature.
    ...
    // 5. Extract Output (use outputs[0] — the model's first output)
